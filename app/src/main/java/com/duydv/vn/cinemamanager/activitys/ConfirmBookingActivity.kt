...
import com.duydv.vn.cinemamanager.model.PaymentMethod
import com.duydv.vn.cinemamanager.model.Room
...
import com.duydv.vn.cinemamanager.model.SlotTime
    ...
    private fun getListTimeLocal(roomId: Int): List<SlotTime> {
        ...
        if (roomFirebase?.times != null) {
            for (timeFirebase in roomFirebase.times!!) {
                val slotTime = SlotTime(
                    timeFirebase.id, timeFirebase.title,
                    false, roomId
                )
                list.add(slotTime)
            }
...
//        }
    ...
    private fun onClickBookingMovie() {
        ...
        if (StringUtil.isEmpty(getTitleRoomSelected())) {
            Toast.makeText(this, getString(R.string.msg_select_room_require), Toast.LENGTH_SHORT)
                .show()
            return
        }
        if (StringUtil.isEmpty(getTitleTimeSelected())) {
            Toast.makeText(this, getString(R.string.msg_select_time_require), Toast.LENGTH_SHORT)
                .show()
            return
        }
    ...
    private fun setListSeatUpdate() {
        for (seatChecked in getListSeatChecked()) {
            getSeatFirebaseFromId(
                seatChecked.roomId,
                seatChecked.timeId, seatChecked.id
            )?.isSelected = true
        }
    }
    ...
    private fun showDialogConfirmBooking() {
        ...
        val strTotalAmount = getTotalAmount().toString() + Constant.UNIT_CURRENCY
        ...
        tvDialogOk.setOnClickListener(object : IOnSingleClickListener() {
            override fun onSingleClick(v: View?) {
                ...
                mBookingHistory = BookingHistory(
                    id, mMovie!!.id, mMovie?.name,
                ...
                if (Constant.PAYMENT_CASH == mPaymentMethodSelected?.type) {
                    sendRequestOrder()
                } else {
    ...
    private fun sendRequestOrder() {
        ...
        MyApplication[this@ConfirmBookingActivity].getMovieDatabaseReference()
            ...
            .setValue(mMovie) { _: DatabaseError?, _: DatabaseReference? ->
                MyApplication[this@ConfirmBookingActivity].getBookingDatabaseReference()
                    ...
                    .setValue(mBookingHistory) { _: DatabaseError?, _: DatabaseReference? ->
                        ...
                        Toast.makeText(
                            ...
                            getString(R.string.msg_booking_movie_success), Toast.LENGTH_LONG
                        ).show()
